...
The local settlement chain detects changes to recorded events. It can be stored in SQLite/WAL or the legacy JSONL chain file. It does not prevent the operator from deleting the store or choosing not to record a request. For real payouts, back up the store and periodically publish the latest hash to an external system such as a public blockchain, timestamping service, or signed transparency log.
